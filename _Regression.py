...
def Regression(self):
    ...
    选择文件夹进行数据公式回归
    其中文件夹命名方式必须为Variable_Radius_Degree
    回归多项式的次数由界面Degree确定
    :param self: class
    ...
    y = data[:, 2]
    print('2' * 10)
    poly_reg = PolynomialFeatures(degree=Degree)  # Quadratic polynomial 二次多项式
    X_ploy = poly_reg.fit_transform(x) # Generate polynomial data  产生多项式数据
    print('2.1' * 10)
    # max parameter
    ...
    score = lin_reg_2.score(X_ploy, y)  ## The model evaluation that comes with sklearn has the same logic as R2_1 sklearn中自带的模型评估，与R2_1逻辑相同
    print('3' * 10)
    var_n = x.shape[1]
    var_str = []
    ...
    # for j in range(3):
    #     name = ['x', 'y', 'z'][j]
    #     y = np.array(positions)[:, j]
    #     exec("lin_reg_%s=linear_model.LinearRegression()" % name)
    #     exec("lin_reg_%s.fit(X_ploy,y)" % name)
    #     exec("predict_y =  lin_reg_%s.predict(X_ploy)" % name)
    #     exec("score = lin_reg_%s.score(X_ploy, y)" % name)  ##sklearn中自带的模型评估，与R2_1逻辑相同
    #     var_n = x.shape[1]
    #     var_str = []
    #     for i in range(1, var_n + 1):
    #         exec("x%d = sy.symbols('x_%d')" % (i, i))
    #         var_str.append('x%d' % i)
    #     print('lin_reg_x.intercept_')
    #     print(lin_reg_x.intercept_)
    #     feature_names_out = poly_reg.get_feature_names_out(var_str)
    #     exec("equation%s = sy.Float(lin_reg_%s.intercept_,4)" % (name, name))
    #     for i in range(len(feature_names_out)):
    #         var = feature_names_out[i].replace('^', '**').replace(' ', '*')
    #         exec("equation%s = equation%s + sy.Float(lin_reg_%s.coef_[i],4) * eval(var)" % (name, name, name))
...
def ComputeR(self, equation, equation_):
    ...
    由回归多项式计算新的半径和角度下的变量的最大值
    :param self: class
    :param equation: Polynomial formulas for radius and Angle 关于半径和角度多项式公式
    :return: Variable maximum 变量最大值
    '''
    print('ComputeR')
